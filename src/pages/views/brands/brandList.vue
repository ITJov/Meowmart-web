<template>
  <VCard>
    ...
    <VCardTitle class="d-flex align-center pa-4">
      <span class="text-h5">Tabel Brand</span>
      <VSpacer />
      <VTextField
        v-model="search"
        density="compact"
...
const fetchBrands = async () => {
  ...
  try {
    ...
    totalBrands.value = data.data.total;
  } catch (error) {
...
const save = async () => {
  ...
  try {
    if (isEditMode.value) {
      ...
      await axios.post(`/api/brands/${editedItem.value.id}`, formData, config);
    } else {
