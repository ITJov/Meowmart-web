<template>
  <VCard>
    <VCardTitle class="d-flex align-center pa-4">
      <span class="text-h5">Tabel Pembayaran</span>
      <VSpacer />
      <VBtn color="orange" @click="downloadReport" :disabled="loading || payments.length === 0">
    ...
    >
      ...
      <template #no-data>
        <div class_v-data-table-server="text-center py-4">
          Tidak ada data pembayaran yang ditemukan untuk filter yang dipilih.
        </div>
      </template>
    </VDataTableServer>
...
// --- Headers Tabel ---
const headers = [
  { title: 'TANGGAL PEMBAYARAN', key: 'payment_date' },
  { title: 'NO. REFERENSI', key: 'reference', sortable: false },
  { title: 'JUMLAH', key: 'amount' },
  { title: 'METODE PEMBAYARAN', key: 'method', sortable: false },
  { title: 'NAMA PELANGGAN', key: 'customer_name', sortable: false },
  { title: 'NO. HP', key: 'customer_phone', sortable: false },
] as const;
...
const fetchReport = async () => {
  if (!isFilterValid.value) {
    // Jangan fetch jika filter dasar (cabang & tanggal) belum siap
    payments.value = [];
    totalItems.value = 0;
    return;
  }
  ...
  try {
    const { data } = await axios.get('/api/reports/payment-report', {
      params: {
    ...
    if (data.success && data.data) {
      payments.value = data.data.data; // Data dari paginator
