<template>
  <VCard>
    <VCardTitle class="d-flex align-center pa-4">
      ...
      <VSpacer />
      <VBtn color="orange" @click="downloadReport" :disabled="loading || salesList.length === 0">
    ...
    >
      ...
      <template #[`item.payment_status`]="{ item }">
        <VChip :color="getStatusColor(item.payment_status)" size="small" variant="tonal">
          {{ item.payment_status }}
        </VChip>
      </template>
      ...
      <template #no-data>
        <div class="text-center py-4">
          Tidak ada data penjualan yang ditemukan untuk filter yang dipilih.
        </div>
      </template>
    </VDataTableServer>
...
// --- Headers Tabel ---
const headers = [
  { title: 'TANGGAL TRANSAKSI', key: 'order_date' },
  { title: 'NO. INVOICE', key: 'invoice_number' },
  { title: 'NAMA PELANGGAN', key: 'customer_name', sortable: false },
  { title: 'TOTAL TRANSAKSI', key: 'total' },
  { title: 'STATUS TRANSAKSI', key: 'payment_status' },
] as const;
...
const getStatusColor = (status: string) => {
  ...
  if (lowerStatus === 'unpaid') return 'error';
  if (lowerStatus === 'partial') return 'warning';
  return 'default';
};
...
const fetchReport = async () => {
  ...
  try {
    const { data } = await axios.get('/api/reports/sales', { // <-- URL API BARU
      params: {
    ...
    if (data.success && data.data) {
      salesList.value = data.data.data; // Data dari paginator
